...
export const useMapStore = create<{
  ...
  mapMode: '2d', // Default to 2D since Indonesia has limited 3D coverage
 ...
 */
export const useApiKeys = create<{
  geminiApiKey: string;
  mapsApiKey: string;
  ...
  mapsApiKey: typeof window !== 'undefined' ? localStorage.getItem('maps_api_key') || '' : '',
  get isConfigured() {
    const state = get();
    return state.geminiApiKey.length > 0 && state.mapsApiKey.length > 0;
  },
