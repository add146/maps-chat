  ...
  return (
    <section className="control-tray" ref={trayRef}>
      <nav className={cn('actions-nav', { 'text-entry-visible-landscape': isLandscape && isTextEntryVisible })}>
        ...
        <button
          className={cn('action-button', { 'connected': isChatVisible })}
          onClick={toggleChat}
          title={isChatVisible ? 'Sembunyikan Chat' : 'Tampilkan Chat'}
          aria-label={isChatVisible ? 'Sembunyikan Chat' : 'Tampilkan Chat'}
        >
          <span className="icon">{isChatVisible ? 'chat' : 'chat_bubble_outline'}</span>
        </button>
        <button
          className={cn('action-button')}
          onClick={() => {
